...
async def main():
    ...
    async with AsyncWebsocketClient("wss://s.devnet.rippletest.net:51233") as client:
        ...
        print("\n=== Cover Available ===\n")
        cover_info = await client.request(LedgerEntry(
        ...
        deposit_response = await submit_and_wait(cover_deposit_tx, client, loan_broker)
        ...
        asset_issuer_info = await client.request(LedgerEntry(
            mpt_issuance=mpt_id,
            ledger_index="validated",
        ))
        ...
        clawback_response = await submit_and_wait(cover_clawback_tx, client, mpt_issuer)
