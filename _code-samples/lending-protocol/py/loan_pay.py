...
async def main():
    ...
    async with AsyncWebsocketClient("wss://s.devnet.rippletest.net:51233") as client:
        ...
        print("\n=== Loan Status ===\n")
        loan_status = await client.request(LedgerEntry(
            index=loan_id,
        ...
        pay_response = await submit_and_wait(loan_pay_tx, client, borrower)
        ...
        delete_response = await submit_and_wait(loan_delete_tx, client, borrower)
        ...
        verify_response = await client.request(LedgerEntry(
            index=loan_id,
            ledger_index="validated",
        ))
